...
class FFMpegRecipe(Recipe):
    ...
    def build_arch(self, arch):
        with current_directory(self.get_build_dir(arch.arch)):
            ...
            # 修复：上游配方此处误用 `=` 覆盖了上面的 `--disable-everything`，
            # 导致 ffmpeg 8.0.1 全量编译所有编解码器 → 内存耗尽崩溃（主 build.log 为空）。
            # 这里改为 `+=`，保留 --disable-everything（只编译 mp4 最小集）。
            flags += [
                '--enable-jni',
                '--enable-mediacodec'
            ]
            ...
            # other flags:
            # 注意：额外加入 atempo 滤镜——本应用倍速播放（带声音、保音调）依赖
            # ffpyplayer 的 ff_opts['af'] = 'atempo=...'。上游配方最小滤镜集里漏了它，
            # 不加的话安卓上切倍速会因找不到 atempo 滤镜而打不开播放器。
            flags += [
                '--enable-filter=aresample,resample,crop,adelay,atempo,volume,scale',
